...
$status_query = "Insert into json_api_bf_status (start_time,status) VALUES ('$current_time',0)";
	...
	// dump json into a file on server 
			 $fileName = 'jsondump_bf';
				 ...
				 if (file_put_contents($fileName,'') !== false)
				 {
					...
					fclose($fp);
					mysqli_query($conn,"INSERT INTO jsondump_bf(filename,created) VALUES('".mysqli_real_escape_string($conn, $fileName)."','$current_time')");
					
				 }else
				 {
					mysqli_query($conn, "INSERT into r1_logs_bf(message,created_at) VALUES ('Zipcodesnew backup could not be uploaded','$current_time')"); 
				 }
...
$status_query = "UPDATE json_api_bf_status SET end_time='$date',status = 1 WHERE id = $last_insert_id";
$status_query_result = mysqli_query($conn,$status_query);
