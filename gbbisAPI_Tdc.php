...
$status_query = "Insert into json_api_tdc_status (start_time,status) VALUES ('$current_time',0)";
...
$last_insert_id	 = mysqli_insert_id($conn);
 	...
 	try{
		...
		if(!empty($Result))
		{
			$sqlTruncate="TRUNCATE TABLE jsondataAPI_tdc";
			mysqli_query($conn,$sqlTruncate);
			foreach($Result as $row)
			{
				
				if($row->Brand == "Driveway Company" || $row->Brand == "The Driveway Company")
				{
					...
					if(!empty($row->ZipCodes))
					{
						foreach($row->ZipCodes as $allzips)
						{
							...
							$sql = "INSERT into jsondataAPI_tdc(TerritoryName,LicenseNumber,OwnedTerritoryParent,ParentLicenseNumber,DonuntParentTerritory,
	...
	// dump json into a file on server 
			 $fileName = 'jsondump_tdc';
				 ...
				 if (file_put_contents($fileName,'') !== false)
				 {
					...
					fclose($fp);
					mysqli_query($conn,"INSERT INTO jsondump_tdc(filename,created) VALUES('".mysqli_real_escape_string($conn, $fileName)."','$current_time')");
					
				 }else
				 {
					mysqli_query($conn, "INSERT into r1_logs_tdc(message,created_at) VALUES ('Zipcodesnew backup could not be uploaded','$current_time')"); 
				 }
...
$status_query = "UPDATE json_api_tdc_status SET end_time='$date',status = 1 WHERE id = $last_insert_id";
$status_query_result = mysqli_query($conn,$status_query);
