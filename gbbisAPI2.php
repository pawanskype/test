...
$check_previous_status  = "Select status from json_api_status order by id desc limit 1";
...
if ($row['status'] == 0) {
	//$object->send_failure_api_msg();
	$object->send_failure_api_msgUsingMail();
	exit();
}
//$query = "select * from jsondataAPI  WHERE ZipCodeName IN ('MONTGOMERY');";
...
if ($flag == TRUE) {
	...
	// fauilure mail send
	$result_sql_restoration1 = mysqli_query($conn, "SELECT ANY_VALUE(parent_license) as parentlicense ,ANY_VALUE(remark) as remarking ,GROUP_CONCAT(zipcode) as zipcodes FROM r1_failure where buss_id = '50' GROUP BY remark");
		...
		while ($rowFetch = mysqli_fetch_array($result_sql_restoration1)) {
			if (isset($sendR1_failuremail) and $sendR1_failuremail == true) {
				...
				<p style="width: 68%; font-size:18px;">
					<span style=" color: rgb(43, 40, 40); font-size: 14px; font-weight: bold; margin-right:10px;">' . $j . '.</span>' . $rowFetch["remarking"] . '
				</p>
				<p style="width: 68%; font-size:18px;">
	...
	$txt = "Code executed at time " . date("Y-m-d h:i");
	$object->testmessageUsingMail("cron finished");
}
die(" gbbisAPI2 Cron run successfully");