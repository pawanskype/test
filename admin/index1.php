	...
	if($email!='' && $password!='') {
		...
		if(mysqli_num_rows($sql)>0) {
			...
			if($user_data['status']==0) {
				...
				header('Location:'.$adminurl.'?error=3');
			}
		}else {
			echo "step 8 =><br>";
			$_SESSION['msg_error'] = "User does not exits.";
			header('Location:'.$adminurl.'?error=2');
		}
	}else {
		echo "step 9 =><br>";
		$_SESSION['msg_error'] = "Both fields are required rrrrrr";
		echo "<pre>";print_r($_SESSION);echo "</pre>";
		//header('Location:'.$adminurl.'?error=1');
	}	
}else {
	echo "step 10 =><br>";
...
echo "<pre>";print_r($_SESSION);echo "</pre>";
?>
<body class="index-body" class="raw">
	<div class="container">
			...
			<div class="form-container-in jim">			
				...
				<form class="index-form" id="login" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
					...
					<div class="form-group">
						...
						<button type="submit" name="login" class="btn btn-login">Login admin</button>
					</div>
				</form>
