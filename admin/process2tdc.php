							...
							{												
								...
								if($sql_run_query){	
									...
									if(!empty($status_datanew))
									{
										...
										//exception for parent;	
										$Remark = $pid1." Driveway Company Parent license is not found in r1_parent_info_lookUp table";
										$location_name='';
										$plicense='';
			...
			try
			{
			   $csvdata = array();		
				while ($row = mysqli_fetch_array($result)){
                 ...
                 $fileName = '../backup/r1_backup/'.$fileName."__tdc".date("Y-m-d_H-i-s",time()).".csv";
				 ...
				 {					 
					...
					fclose($fp);
					mysqli_query($conn,"INSERT INTO r1_backup_tdc(file_name,brand,created_at) VALUES('".mysqli_real_escape_string($conn, $fileName)."','Driveway Company','".$current_time."')");
					...
					if(mysqli_query($conn,$sql))
					{
						...
						$newFilePath = "../store-locator-sitemap/sitemap-backup/sitemap_tdc".date("Y-m-d_H-i-s",time()).".xml";

						$fileMoved = rename($currentFilePath, $newFilePath);		//move file to new backup location
						if($fileMoved)
						{
						  $fileName = '../../store-locator-R1.xml';
						  $mysqlquery = mysqli_query($conn, "select z.zipcode,z.city from zipcodesnew_tdc z UNION select m.zipcode,m.city from markers m where m.adminid=$buss_id");
					...
					}else
					{
						...
						mysqli_query($conn, "INSERT into r1_logs_tdc(message,created_at) VALUES ('R1 sitemap could not be created as zipcodesnew_tdc table not updated','".$current_time."')");
					}
					
				 }else
				 {
					  $r1_messages[] = "zipcodesnew_tdc backup could not be uploaded and all Zipcodesnew process was ommitted.";
				    mysqli_query($conn, "INSERT into r1_logs_tdc(message,created_at) VALUES ('zipcodesnew_tdc backup could not be uploaded','".$current_time."')");	 
				 } 				 
			}catch(Exception $e) 
			{
				//Insert data in log table and check exception					
				mysqli_query($conn, "INSERT into r1_logs_tdc(message,created_at) VALUES ('".mysqli_real_escape_string($conn, $e->getMessage())."','".$current_time."')");
			}
		}else
		{
		 ...
		 {
			// making stats
			$json_r1_owned = mysqli_num_rows(mysqli_query($conn, 'select * from jsondataAPI_tdc where Brand =  "'.$brand_name.'"  and Status = "Owned"'));		      
			$json_r1_donut = mysqli_num_rows(mysqli_query($conn, 'select * from jsondataAPI_tdc where Brand =  "'.$brand_name.'"  and Status = "Donut"'));
		    if($zipcodesnewprocess == true)
			  {
				$db_r1_owned = mysqli_num_rows(mysqli_query($conn, 'select * from r1_bridge_tdc where  brand =  "'.$brand_name.'"  and status = "Owned"'));
			  
			    $db_r1_donut = mysqli_num_rows(mysqli_query($conn, 'select * from r1_bridge_tdc where brand =  "'.$brand_name.'"  and status = "Donut"'));

                $db_r1_owned_failures = mysqli_num_rows(mysqli_query($conn, 'select * from r1_failure_tdc where brand =  "'.$brand_name.'"  and status = "Owned"'));
			  
			    $db_r1_donut_failures = mysqli_num_rows(mysqli_query($conn, 'select * from r1_failure_tdc where brand =  "'.$brand_name.'"  and status = "Donut"'));				
			  }else
			  {
				$db_r1_owned = 0;			  
			    $db_r1_donut = 0;
			  ...
			  $insertsql = "INSERT INTO gbbis_stats_tdc(json_r1_owned,json_r1_donut,database_r1_owned,database_r1_donut,database_r1_owned_failures,database_r1_donut_failures,created) VALUES ('".$json_r1_owned."','".$json_r1_donut."','".$db_r1_owned."','".$db_r1_donut."','".$db_r1_owned_failures."','".$db_r1_donut_failures."','".date("Y-m-d H:i:s")."')";
			  mysqli_query($conn,$insertsql);
		 }	
        
		// fauilure mail send 
	   $result_sql_restoration1 = mysqli_query($conn,"SELECT parent_license,remark,GROUP_CONCAT(zipcode) as zipcodes FROM r1_failure_tdc where buss_id = '$buss_id' GROUP BY remark");		
