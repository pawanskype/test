          ...
          <div class="content-wrapper">
            <h1 class="page-title">Logs</h1>
			 <?php if(isset($_SESSION['msg_error'])) { ?>
				...
				<?php if($role == 'superadmin') { ?>
					...
					<select class="locationchange">
						<?php $i = 0; while($selectdata = mysqli_fetch_assoc($selectadmin)) {
							$i++;
							if($selectdata['adminid']==49 || $selectdata['adminid']==50){
								echo '<option value='.$adminurl.'/r1logs?adminid='.$selectdata['adminid'].''.(($selectdata['adminid']==$_GET['adminid'])?' selected':'').'>'.$selectdata['business'].'</option>';
							}
							$adminid = 50;
						}
						?>
				...
				<?php if($_GET['adminid'] =="") { ?>
				<script>
					var currentbusiness = "<?php echo $adminurl.'/r1logs?adminid='.$adminid; ?>";
			...
			<!--<div id="multiple-marker-map"></div>-->
              <div id="location-list">
				...
				<tbody>
					<tr>
						<td>Begin Date:</td>
						<td><input type="text" id="min" name="min" data-date-format='dd-mm-yy' class="form-control datepicker" value="" placeholder="Enter Begin Date" /></td>
						<td style="padding-left:10px;">End Date:</td>
						<td><input type="text" id="max" name="max" data-date-format='dd-mm-yy' class="form-control datepicker" value="" placeholder="Enter End Date " /></td>
				...
				<br>
					<div class="table-responsive">
						...
						<!--<input type="checkbox" id="checkAll" class="custom-check">-->
							...
							<tbody class="list">
							<?php 
								$query = mysqli_query($con,"SELECT * from r1_logs"); 
								if(!empty($query) && mysqli_num_rows($query)>0 ){	
									while($row = mysqli_fetch_assoc($query)){
										?>
										<tr>
											<td class="location-zipcode"><?php echo $row['message']; ?></td>											
