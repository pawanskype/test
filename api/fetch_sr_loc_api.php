...
$adminid = 53;  // id for sc admin
$base_url = 'https://softroc.com/wp-content';
$status = 0;
$output = [];
$statusCode = 400;
$api = new restApi();
$zipdata['zipscodes'] = [];
$zipcode_tbl = 'zipcodesnew_sr';
$output = [];
if (!empty($_REQUEST['search'])) {
    ...
    if (preg_match('/[a-zA-Z]/i', $search_parameter)) {
        if (strlen($search_parameter) > 2) {
            ...
            $ch = curl_init();
            $url = 'https://softroc.com/softroc-store-locator-api/?state='.$state.'&token=$2y$07$BCryptRequires22Chrcte/VlQH0piJtjXl.0t1XkA8pw9dMXTpOq';
            ...
            mysqli_real_escape_string($conn, $radius));
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = @mysqli_fetch_assoc($result)) {
                        ...
                        } elseif (substr($search_parameter, 0, 1) != '0' && strlen($search_parameter) == 3) {
                        ...
                        $output[] = ['zipnewId' => $zipnewId, 'id' => $row['id'],
                 'adminid' => $row['adminid'],
                 'slug' => $row['slug'],
                 'name' => $row['name'],
                // "address"=> $row['address'],
