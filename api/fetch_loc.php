...
$zipcode_tbl = 'zipcodesnew';
$output=array();
if(!empty($_REQUEST['search']) ){
	$search_parameter=$_REQUEST['search'];
	//its state codeNot any post found
	if(preg_match("/[a-zA-Z]/i", $search_parameter)){
        if (strlen($search_parameter) > 2) {
            ...
            $ch = curl_init();
            $url = 'https://restoration1.com/store-locator-api/?state='.$state.'&token=$2y$07$BCryptRequires22Chrcte/VlQH0piJtjXl.0t1XkA8pw9dMXTpOq';
		...
		//$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($search_parameter)."&sensor=false&key=AIzaSyCcfvCwbcQmmGDFC98uCak_BuvJjOxvJw8";
		...
		if(empty($result['results'])){
			...
			$query = sprintf("SELECT z.id as ID,z.zip_name,z.zipcode as newzip,z.city,z.state,z.country,z.lat newlat,z.lng newlng,m.address,m.phone, m.id, m.adminid, m.slug, m.name,m.zipcode,m.addressshow FROM markers m INNER JOIN zipcodesnew z on m.id=z.loc_id  WHERE m.adminid='$adminid' and FIND_IN_SET('$search_parameter',z.zipcode) > 0",
		  ...
		  if (!$result) {
			...
			if(strpos($phone, '-') !== false)
			{
			 ...
			 if (substr($search_parameter, 0, 1) != '0' && strlen($search_parameter)==4) {
				 ...
				 $purl=get_State_PageUrl($row['state'],$row['name'], $conn);
             if(mysqli_num_rows($result) > 1)
				...
				$term_id= findState_SlugTerm($state,$conn);
				if($term_id != '' ){
				...
				$serverdarea=get_commonserved_area($area_zipcode,$conn);
				...
				// "address"=> $row['address'],
				 ...
				 "common_serverdarea" => $serverdarea,
				 "state"=> $row['state'],
				 "phone"=>$phone_no,
				 "lat"=>$lat,
				 "lng"=> $lng,
			...
			$api->response($api->json($output,'success',200,1), 200);
			 ...
			 if(mysqli_num_rows($result)>0 ){
				...
				while ($row = @mysqli_fetch_assoc($result)){
					...
					$serverdarea=get_commonserved_area($row['zipcode'],$conn);
					$purl=get_State_PageUrl($row['state'],$row['name'], $conn);
