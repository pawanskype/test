...
if (!empty($_REQUEST['search'])) {
    ...
    if (preg_match("/[a-zA-Z]/i", $search_parameter)) {
        if (strlen($search_parameter) > 2) {
            ...
            $state = str_replace(" ", "%20", strtolower($state));
            $bf_apiurl = "https://bluefrogplumbing.com/bf-store-locator-api/?state=" . $state . '&token=$2y$07$BCryptRequires22Chrcte/VlQH0piJtjXl.0t1XkA8pw9dMXTpOq';
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_URL, $bf_apiurl);
            $output = curl_exec($ch);
        ...
        if (empty($result['results'])) {
            ...
            $query = sprintf(
                "SELECT z.id as ID,z.zip_name,z.zipcode as newzip,z.city,z.state,z.country,z.lat newlat,z.lng newlng,m.address,m.phone, m.id, m.adminid, m.slug, m.name,m.zipcode,m.addressshow FROM markers m INNER JOIN " . $zipcode_tbl . " z on m.id=z.loc_id  WHERE m.adminid='$adminid' and FIND_IN_SET('$search_parameter',z.zipcode) > 0",
            ...
            if (mysqli_num_rows($result) > 0) {
				...
				*/
                ...
                $query = sprintf(
                    "SELECT address, city, state, phone, id, adminid, slug, name,zipcode,addressshow, lat, lng, ( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM markers WHERE adminid='$adminid' AND zipcode='$search_parameter' HAVING distance < %s ORDER BY distance",
                    mysqli_real_escape_string($conn, $center_lat),
                    mysqli_real_escape_string($conn, $center_lng),
                    mysqli_real_escape_string($conn, $center_lat),
                    mysqli_real_escape_string($conn, $radius)
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $query = sprintf(
                        "SELECT address, city, state, phone, id, adminid, slug, name,zipcode,addressshow, lat, lng, ( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM markers WHERE adminid='$adminid' HAVING distance < %s ORDER BY distance LIMIT 0 , 10",
                        mysqli_real_escape_string($conn, $center_lat),
                        mysqli_real_escape_string($conn, $center_lng),
                        mysqli_real_escape_string($conn, $center_lat),
                        mysqli_real_escape_string($conn, $radius)
