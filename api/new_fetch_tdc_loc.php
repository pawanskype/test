...
if(!empty($_REQUEST['search']) ){	
	...
	if(preg_match("/[a-zA-Z]/i", $search_parameter)){
		if(strlen($search_parameter)> 2){
			...
			$resulttdcSql = mysqli_query($conn, "SELECT zipcode FROM markers WHERE state = '".$state."' AND adminid = 52");
           
			//$datatdc = $resulttdcSql->fetch_assoc();
            // Check if the query was successful
          // Check if the SQL query was successful
            if ($resulttdcSql) {
                // Initialize an array to store zipcodes
                $zipcodes = array();

                // Fetch all rows and extract zipcodes
                while ($row = $resulttdcSql->fetch_assoc()) {
                    // Check if the 'zipcode' key exists in the current row
                    if (isset($row['zipcode'])) {
                        // Add the zipcode to the $zipcodes array
                        $zipcodes[] = $row['zipcode'];
                    } else {
                        // Handle the case where 'zipcode' key is not found
                        echo "Warning: Undefined array key 'zipcode'.";
                    }
                }

                // Free the result set
                mysqli_free_result($resulttdcSql);

                // Define the $resulttdc array in the desired JSON structure
                $resulttdc = array(
                    'status' => 'success',
                    'result' => array(
                        'zipscodes' => $zipcodes,
                        'url' => array_fill(0, count($zipcodes), null) // Fill 'url' array with null values
                    )
                );

                // Encode the $resulttdc array to JSON format
                $json_response = json_encode($resulttdc);
                
            } else {
                // Handle the case where the SQL query failed
                $error_message = mysqli_error($conn);
                echo "Failed to execute query: $error_message";
            }
	...
	}elseif((strlen($search_parameter) < 5) || (strlen($search_parameter) > 6)){
