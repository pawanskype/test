...
//if($api->checkAuth()){
	...
	if(!empty($_REQUEST['zipcode'])){	
		...
		if(empty($result['results'])){
			...
			$query = sprintf("SELECT m.emailmesageid as email,m.id as ID,z.r1_number,z.parent_loc_name,m.zipcode as zipcode, m.lat,  m.lng, ( 6371 * acos( cos( radians('%s') ) * cos( radians(  m.lat ) ) * cos( radians(  m.lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( m.lat ) ) ) ) AS distance  FROM markers m INNER JOIN zipcodesnew z on m.id=z.loc_id  WHERE m.adminid='$adminid' and FIND_IN_SET('$zipcode',z.zipcode) > 0 HAVING distance < %s ORDER BY distance",
...
{
	$franchiseData=array();
	$q2="select r1_number,parent_loc_name from zipcodesnew where loc_id='".$marker_Id."' and r1_number!='' limit 1";
